...
void HC_05_init(void)
{
  ...
  if (!Wait_for("+NAME:Temp_control\r\nOK\r\n"))
  {
    ...
    HAL_Delay (50);
    Uart_sendstring("AT+NAME=Temp_control\r\n");
    HAL_Delay (50);
    Wait_for("OK\r\n");
    HAL_Delay (50);
    ...
    Wait_for("OK\r\n");
    HAL_Delay (50);
    //Устанавливаем параметры сканирования для снижения энергопотребления
    Uart_sendstring("AT+IPSCAN=1024,1,1024,1\r\n");
    HAL_Delay (50);
    Wait_for("OK\r\n");
    goto repeat1;
  }
  ...
  //Проверяем параметры сканирования
  HAL_Delay (50);
  Uart_sendstring("AT+IPSCAN?\r\n");
  HAL_Delay (50);
  if (!Wait_for("+IPSCAN:1024,1,1024,1\r\nOK\r\n"))
  {
    goto repeat3;
  }
