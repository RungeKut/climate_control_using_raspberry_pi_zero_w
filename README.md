...
<ol>
    ...
    <li><a href="#установка-сервера-NGINX-в-jail-на-сервер-truenas">Установка сервера NGINX в jail на сервер TrueNAS</a></li>
</ol>
...
После года использования обычного мешкового фильтра, я заметил налет на крыльчатке вентилятора, который стоит сразу после него. Этот слой очень тонкий, но хотелось бы и его не иметь.

Исходя из всего вышеизложенного я пока остановлюсь на бумажном фильтре для пылесоса класса М [HEPA фильтр MAXX 00_11P_00](https://www.ozon.ru/product/hepa-filtr-maxx-00-11p-00-944891213/?advert=_ZGocWnG7rLAubeZOvLzm17onyOOT35BQaueqVM6Kt1jIefOcc02x6BfAPc8eOTaVZgvg9kbGUs_LCINevw7pii7KvFgKAFxpr7u5up9hPzguym3c71t2R2FQf-CKw0bd_jAkZhepZgZ9OZigOoePu7I0kTaQGAWFs_s9GyUABfRlzVYUvTgzKujJeb5qz7OHIok60tzjx9CJVpQcHb9iw&avtc=1&avte=2&avts=1683051923&sh=qpozlgy9Lg) для пылесоса Bosch GAS 35 M,L,AFC. Погоняю вентиляцию с ним...

Прошел год ...

Итак, ровно год и один месяц с момента установки. И по ощущениям сопротивление фильтра выросло раза в два. Раньше мощность вентилятора раньше не выходила за 30% , а сейчас и 70% не хватает. Давления на всасывание вентилятора не достаточно - он работает не эффективно. А ставить его перед фильтром я не очень хочу, т.к. на нем намотается много всякой хрени, зимой он будет работать при отрицательных температурах(сейчас он еще после теплообменника стоит). Коро

Вентилятор сейчас установлен PROMvent ВКК-160 ПЛ. Нужно наверное возвращать 120й, у него давление выше было.

Так же оказалось нужен еще и [электрический подогрев](https://aliexpress.ru/item/1005003735660961.html?spm=a2g2w.orderdetail.0.0.51b74aa6Bju9fq), т.к. получилось так что радиатор отопителя от Газель Бизнес hf730242 был порван когда отключили отопление, а на улице стал минус.
...
<h2 id="A12" align="center">Установка сервера NGINX в jail на сервер TrueNAS</h2>

Добавляем как обычно jail через Gui. Я выбрал 13.2-RELEASE-p2. Базовый. Настроил сеть и подождал пока все установится.

Ставим pkg, набрав просто в Командном интерпретаторе pkg.

Далее ставим текстовый редактор NANO:
...
В Файле nano /etc/rc.conf не забываем добавить:
...
Следующая команда обновит ваши локальные пакеты, а затем установит пакет nginx:

    sudo pkg install nginx

Веб-сервер Nginx установлен, но еще не запущен.

Проверим параметр grep rcvar /usr/local/etc/rc.d/*
...
Это подтверждает, что Nginx установлен и работает правильно, и что его трафик проходит через брандмауэр.
